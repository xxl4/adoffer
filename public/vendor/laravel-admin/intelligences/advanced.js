var advanced = {
	...
	get_to_date_clients : function(event){
		...
		if(date_end == date_start){
			...
			var time_report = date_end +"-"+date_start;
		}
		// if(event){
	...
	export_exel : function(){
		$("#btnExport").click(function(e) {
			e.preventDefault();
			//exporter
			var date_end = $('.date_end').val().toString().replace(new RegExp('/', 'g'), '_');
			var date_start = $('.date_start').val().toString().replace(new RegExp('/', 'g'), '_');
			var date = date_start==date_end?date_start:date_start+'-'+date_end;

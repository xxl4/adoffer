 	...
 	var Mapplic = function() {
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				this.title = $('<h4 style="display: inline-block;"></h4>').addClass('mapplic-tooltip-title').appendTo(this.el);
			...
			this.show = function(location) {
				if (location) {
					...
					var flag = "<div style='width:30px; display:inline-block; vertical-align: middle;'><img src='images/flags/"+location.title.replace(/ /g,'-')+"-Flag.png'></div>";
					var title = "<div style='display: inline-block; vertical-align: middle; margin: 0 5px;'>"+location.title+"</div>";
					this.title.html(flag+title);
					this.desc.html(location.description);

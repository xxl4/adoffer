var Rickshaw={
	namespace:function(namespace,obj){
		...
		for(var i=1,length=parts.length;i<length;i++){
			var currentPart=parts[i];parent[currentPart]=parent[currentPart]||{};
	...
	extend:function(destination,source){for(var property in source){
...
if(typeof module!=="undefined"&&module.exports){
	...
	function keys(object){if(Type(object)!==OBJECT_TYPE){
		throw new TypeError
	}
	var results=[];
...
Rickshaw.Graph=function(args){
	...
	this.element=args.element;this.series=args.series;this.defaults={
	...
	this.initialize=function(args){
		...
		this.vis=d3.select(this.element).append("svg:svg").attr("width",'100%').attr("height",this.height);
		for(var name in Rickshaw.Graph.Renderer){
			...
			self.registerRenderer(new r({graph:self}))}this.setRenderer(args.renderer||"stack",args);
			this.discoverRange()
		};
		this.validateSeries=function(series){
			if(!Array.isArray(series)&&!(series instanceof Rickshaw.Series)){
				var seriesSignature=Object.prototype.toString.apply(series);
				throw"series is not an array: "+seriesSignature}var pointsCount;
				series.forEach(function(s){
					...
					if(s.data.length>=3){if(s.data[2].x<s.data[1].x||s.data[1].x<s.data[0].x||s.data[s.data.length-1].x<s.data[0].x){
			...
			this.dataDomain=function(){
				...
				var min=d3.min(data.map(function(d){
					return d[0].x}));var max=d3.max(data.map(function(d){return d[d.length-1].x
					}));
					return[min,max]
				};
				...
				this.update=this.render;this.stackData=function(){
					...
					stackedData=stackedData||data;this.stackData.hooks.after.forEach(function(entry){
		...
		this.setSize=function(args){args=args||{};
	...
	Rickshaw.Fixtures.RandomData=function(timeInterval){
		var addData;timeInterval=timeInterval||1;
		var lastRandomValue=200;
		var timeBase=Math.floor((new Date).getTime()/1e3);
		this.addData=function(data){
			var randomValue=Math.random()*100+15+lastRandomValue;
			var index=data[0].length;var counter=1;
	...
	Rickshaw.Fixtures.Time=function(){
		...
		var self=this;this.months=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
		this.units=[{
		...
		this.unit=function(unitName){
			...
			Rickshaw.Fixtures.Time.Local=function(){
				var tzOffset=(new Date).getTimezoneOffset()*60;var self=this;
				...
				this.units=[{
					...
					name:"second",
					seconds:1,formatter:function(d){
						return d.getSeconds()+"s"
					}
				}];
				this.unit=function(unitName){
			...
			Rickshaw.Fixtures.Number.formatKMBT=function(y){
				...
				if(abs_y>=1e12){
					...
					return y/1e9+"B"
				}else if(abs_y>=1e6){return y/1e6+"M"
			}else if(abs_y>=1e3){
				return y/1e3+"K"
			}else if(abs_y<1&&y>0){
				return y.toFixed(2)
				...
				Rickshaw.keys(self.data).forEach(function(time){
					...
					annotation.boxes.forEach(function(box){
						...
						if(!element){
							...
							element.innerHTML=box.content;annotation.element.appendChild(element);
		...
		Rickshaw.Graph.Axis.Time=function(args){
			...
			this.tickOffsets=function(){
				...
				for(var i=0;i<count;i++){
					...
					runningTick=tickValue+unit.seconds/2;offsets.push({value:tickValue,unit:unit})
				}return offsets
			};
			this.render=function(){
			...
			Rickshaw.Graph.Axis.X=function(args){
				...
				var berthRate=.1;
				this.initialize=function(args){
					this.graph=args.graph;this.orientation=args.orientation||"top";
				...
				this.setSize=function(args){
					...
					this.render=function(){
						...
						this._discoverSize=function(element,args){
							...
							this.initialize(args)};Rickshaw.namespace("Rickshaw.Graph.Axis.Y");
							Rickshaw.Graph.Axis.Y=Rickshaw.Class.create({
								...
								render:function(){
									...
									this._drawGrid(axis);this._renderHeight=this.graph.height
								},
									...
									l.element.addEventListener("mouseout",function(e){
										...
										self.legend.lines.forEach(function(line){
											if(l===line&&line.hasOwnProperty("originalIndex")){
												...
												delete line.originalIndex}if(colorSafe[line.series.name]){
							...
							Rickshaw.Graph.Behavior.Series.Order=function(args){
								this.graph=args.graph;this.legend=args.legend;
								var self=this;if(typeof window.$=="undefined"){
								...
								this.graph.onUpdate(function(){
									var h=window.getComputedStyle(self.legend.element).height;self.legend.element.style.height=h
								})
							};
							...
							Rickshaw.Graph.Behavior.Series.Toggle=function(args){
								this.graph=args.graph;
								this.legend=args.legend;
								var self=this;this.addAnchor=function(line){
									...
									anchor.onclick=function(e){
										if(line.series.disabled){
											...
											if(this.graph.series.filter(function(s){
												return!s.disabled}).length<=1)return;
												line.series.disable();
									...
									this._addBehavior=function(){
										this.graph.series.forEach(function(s){
											s.disable=function(){
												...
												s.disabled=true;self.graph.update()
											};
											s.enable=function(){
												s.disabled=false;
												self.graph.update()
								...
								Rickshaw.Graph.HoverDetail=Rickshaw.Class.create({
									...
									update:function(e){
										...
										this.graph.series.active().forEach(function(series){
											...
											var point={
												...
												series:series,value:value,distance:distance,order:j,name:series.name
											};
											if(!nearestPoint||distance<nearestPoint.distance){
												nearestPoint=point}points.push(point)
											},this);
									...
									render:function(args){
										...
										dot.className="dot";
										dot.style.top=item.style.top;dot.style.borderColor=series.color;
...
Rickshaw.Graph.Legend=function(args){
	...
	Rickshaw.Graph.RangeSlider=Rickshaw.Class.create({
		...
		build:function(){
			...
			$(function(){
				$(element).slider({
					...
					slide:function(event,ui){
						if(ui.values[1]<=ui.values[0])return;graph.window.xMin=ui.values[0];
			...
			if(graph.window.xMax==null){
				values[1]=domain[1]}$(element).slider("option","values",values)
			}
		});
	...
	Rickshaw.Graph.Renderer=Rickshaw.Class.create({
		...
		domain:function(data){
			...
			stackedData.forEach(function(series){
				...
				if(series[0].x<xMin)xMin=series[0].x;if(series[series.length-1].x>xMax)xMax=series[series.length-1].x
			});
			xMin-=(xMax-xMin)*this.padding.left;
			xMax+=(xMax-xMin)*this.padding.right;
			yMin=this.graph.min==="auto"?yMin:this.graph.min||0;yMax=this.graph.max===undefined?yMax:this.graph.max;
		...
		render:function(args){
			...
			var vis=args.vis||graph.vis;vis.selectAll("*").remove();
			...
			var i=0;series.forEach(function(series){
				if(series.disabled)return;
		...
		configure:function(args){
			...
			Rickshaw.keys(this.defaults()).forEach(function(key){
				...
				if(typeof this.defaults()[key]=="object"){
					Rickshaw.keys(this.defaults()[key]).forEach(function(k){this[key][k]=args[key][k]!==undefined?args[key][k]:this[key][k]!==undefined?this[key][k]:this.defaults()[key][k]},this)
				}else{
	...
	Rickshaw.Graph.Renderer.Bar=Rickshaw.Class.create(Rickshaw.Graph.Renderer,{
		name:"bar",
		defaults:function($super){
			var defaults=Rickshaw.extend(
				$super(),{
					gapSize:.05,unstack:false
				});
		...
		initialize:function($super,args){
			args=args||{};
			this.gapSize=args.gapSize||this.gapSize;$super(args)
		},
		domain:function($super){var domain=$super();
	...
	Rickshaw.Graph.Renderer.Area=Rickshaw.Class.create(Rickshaw.Graph.Renderer,{
		...
		render:function(){
			...
			if(this.stroke){nodes.append("svg:path").attr("d",this.seriesStrokeFactory()).attr("class","line")}
				var i=0;graph.series.forEach(function(series){
					if(series.disabled)return;
					series.path=nodes[0][i++];
					this._styleSeries(series)
				},this)
		...
		initialize:function($super,args){
			...
			series.forEach(function(series){
				...
				var nodes=vis.selectAll("path").data(series.stack.filter(function(d){
					...
					return graph.y(d.y)}).attr("r",function(d){
		...
		domain:function($super){
			...
			_groups:function(){
				...
				Object.keys(renderGroups).forEach(function(key){
					var group=renderGroups[key];groups.push(group)
				});
				return groups
			},
	...
	Rickshaw.Graph.Renderer.LinePlot=Rickshaw.Class.create(Rickshaw.Graph.Renderer,{
		...
		_renderDots:function(){
			var graph=this.graph;graph.series.forEach(function(series){
				if(series.disabled)return;
	...
	Rickshaw.Series.fill=function(series,fill){
		...
		while(i<Math.max.apply(null,data.map(function(d){
			...
			data.forEach(function(d){if(!d[i]||d[i].x!=x){d.splice(i,0,{x:x,y:fill})}});i++
		}
	};
	...
	Rickshaw.Series.FixedDuration=Rickshaw.Class.create(Rickshaw.Series,{
		initialize:function(data,palette,options){
			...
			if(data&&typeof data=="object"&&Array.isArray(data)){
				...
				this.currentSize+=1;this.currentIndex+=1
			}
